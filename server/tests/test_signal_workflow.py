...
def test_accepted_evidence_becomes_grounded_client_normalized_reviewable_signal(test_session_factory):
    ...
    accepted = review_signal(signal.signal_id, "ACCEPTED", expected_version=signal.aggregate_version)
    assert accepted.lifecycle_status == "ACTIVE"
    assert accepted.review_status == "ACCEPTED"
    assert accepted.aggregate_version == signal.aggregate_version + 1
    with pytest.raises(ValueError, match="version conflict"):
        review_signal(signal.signal_id, "REJECTED", expected_version=signal.aggregate_version)
    assert accepted.id == signal.id
    assert accepted.version == signal.version
